...
async def get(user_id: int) -> UserSchema | None:
    """Retrieves an 'User' object based on the provided ID."""
    return await service.get(user_id)

# GET /user
@user_router.get('/', status_code=status.HTTP_200_OK)
async def get_all() -> Sequence[UserSchema]:
    """Retrieves all 'User' objects."""
    return await service.get_all()

# POST /user
@user_router.post('/', status_code=status.HTTP_201_CREATED)
async def create(user_request: UserSchema) -> UserSchema:
...
async def update(user_request: UserSchema, user_id: int) -> UserSchema:
